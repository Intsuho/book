...
<head>
    ...
    <script type="text/javascript" src="static/script/jquery-1.7.2.js">
        $(function () {
            //在事件的function函数中，有一个this对象。这个this对象对应当前正在响应事件的dom对象
            //给删除a标签绑定单击事件，用于删除的确认提示操作
    ...
    <div id="page_nav">
        ...
        <c:when test="${requestScope.page.pageTotal > 5}">
            ...
            <c:if test="${i != requestScope.page.pageNo}"><a
                href="manager/bookServlet?action=page&pageNo=${i}">${i}<a/></c:if>
            </c:forEach>
        ...
        <script>
            $(function () {
                ...
                $("#searchPageBtn").click(function () {
                    ...
                    //href属性可读可写
                    location.herf = "${pageScope.basePath}manager/bookServlet?action=page&pageNo=" + pageNo;
                })
            })
        </script>
    </div>
